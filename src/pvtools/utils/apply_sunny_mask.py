...
def apply_sunny_mask(
    ...
    df_mask = load_dataframe_from_csv(Path(save_dir / "calculated_data" / data_filename / f"{sanitize_filename(sensor_name_ref)}_sunny_periods.csv"))
    ...
    if save_dir is not None:
        ...
        output_path = Path(save_dir / "filtered" / "sunny_periods" / (data_filename + ".csv"))
